      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowForm(false)}>
          <div className="bg-white w-full max-w-[420px] p-[32px] mx-[20px]" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="space-y-[16px]" onSubmit={(e) => { e.preventDefault(); setShowForm(false); }}>
              <div>
                <label className="block text-sm font-sans text-gray-600 mb-[6px]">カード番号</label>
                <input type="text" placeholder="0000 0000 0000 0000" maxLength={19} className="w-full px-[14px] py-[12px] border border-gray-200 text-sm font-sans text-black placeholder:text-gray-400 focus:outline-none focus:border-black transition-colors" />
              </div>
              <div className="grid grid-cols-2 gap-[12px]">
                <div>
                  <label className="block text-sm font-sans text-gray-600 mb-[6px]">有効期限</label>
                  <input type="text" placeholder="MM/YY" maxLength={5} className="w-full px-[14px] py-[12px] border border-gray-200 text-sm font-sans text-black placeholder:text-gray-400 focus:outline-none focus:border-black transition-colors" />
                </div>
                <div>
                  <label className="block text-sm font-sans text-gray-600 mb-[6px]">CVC</label>
                  <input type="text" placeholder="000" maxLength={4} className="w-full px-[14px] py-[12px] border border-gray-200 text-sm font-sans text-black placeholder:text-gray-400 focus:outline-none focus:border-black transition-colors" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-sans text-gray-600 mb-[6px]">カード名義</label>
                <input type="text" placeholder="TARO TANAKA" className="w-full px-[14px] py-[12px] border border-gray-200 text-sm font-sans text-black placeholder:text-gray-400 focus:outline-none focus:border-black transition-colors" />
              </div>
              <div className="flex gap-[12px] pt-[8px]">
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 border border-gray-200 py-[12px] text-sm font-sans text-gray-600 hover:bg-gray-50 transition-colors">
                  キャンセル
                </button>
                <button type="submit" className="flex-1 bg-black text-white py-[12px] text-sm font-sans font-medium hover:bg-gray-900 transition-colors">
                  追加する
                </button>
              </div>
            </form>
