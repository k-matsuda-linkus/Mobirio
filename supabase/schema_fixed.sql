-- ==========================================
-- Mobirio レンタルバイクプラットフォーム
-- ベーススキーマ（修正版）
-- 2026-02-18
-- ==========================================
-- 修正点:
--   1. reservations.payment_id の前方参照を解消（payment_enhancement で削除予定のため除外）
--   2. options テーブルの CREATE TABLE を追加
--   3. btree_gist 拡張を追加（tstzrange インデックス用）
-- ==========================================
...
-- ==========================================
-- 8. 予約テーブル（payment_id は除外 → payment_enhancement で 1:N 対応済み）
-- ==========================================
CREATE TABLE public.reservations (
  ...
  cdw_amount INTEGER DEFAULT 0,
  noc_amount INTEGER DEFAULT 0,
...
CREATE INDEX idx_options_vendor_id ON options(vendor_id);
...
CREATE POLICY "Vendors can manage own options"
  ON options FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM vendors
      WHERE vendors.id = options.vendor_id
      AND vendors.user_id = auth.uid()
    )
  );

-- vendor_closures
CREATE POLICY "Vendors can manage own closures"
  ON vendor_closures FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM vendors
      WHERE vendors.id = vendor_closures.vendor_id
      AND vendors.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Vendors can manage own announcements"
  ON vendor_announcements FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM vendors
...
CREATE POLICY "Vendors can manage own shop reviews"
  ON shop_reviews FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM vendors
      WHERE vendors.id = shop_reviews.vendor_id
      AND vendors.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Vendors can manage own inquiries"
  ON vendor_inquiries FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM vendors
...
-- page_views（書き込みは全員可、閲覧はベンダーと管理者のみ）
CREATE POLICY "Anyone can insert page views"
  ON page_views FOR INSERT
  WITH CHECK (true);

CREATE POLICY "Vendors can view own page views"
  ON page_views FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM vendors
      WHERE (vendors.id = page_views.vendor_id OR page_views.bike_id IN (SELECT id FROM bikes WHERE bikes.vendor_id = vendors.id))
      AND vendors.user_id = auth.uid()
    )
  );
